  ...
  return (
    <div className="flex-1 bg-[#f7f7f5] min-h-screen px-10 py-12 flex flex-col gap-10 max-w-4xl">
      ...
      <div>
        ...
        <p className="text-[14px] text-slate-400 font-medium">Track the real-time synchronization of your network installation.</p>
      </div>
        ...
        <>
          ...
          {installation.status === 'assigned' && (
            <div className="bg-blue-50 border border-blue-100 rounded-2xl p-6 flex items-start gap-4 animate-in fade-in slide-in-from-top-2">
              <div>
                <p className="text-blue-900 font-bold text-[14px] mb-1 uppercase tracking-wider">Field Engineer Allocated</p>
                <p className="text-blue-700/70 text-[13px] leading-relaxed">
                  <strong className="text-blue-900 font-semibold">{installation.technician?.name}</strong> has been dispatched. They will verify your site protocols before arrival.
                </p>
              </div>
            </div>
          )}
          ...
          {installation.status === 'in_progress' && (
            <div className="bg-amber-50 border border-amber-100 rounded-2xl p-6 flex items-start gap-4">
              <div>
                <p className="text-amber-900 font-bold text-[14px] mb-1 uppercase tracking-wider">Configuration In Progress</p>
                <p className="text-amber-700/70 text-[13px] leading-relaxed">
                   Hardware sync is active. <strong className="text-amber-900 font-semibold">{installation.technician?.name}</strong> is currently finalizing your node setup.
                </p>
              </div>
            </div>
          )}
          ...
          {installation.status === 'completed' && (
            <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-6 flex items-start gap-4">
              <div>
                <p className="text-emerald-900 font-bold text-[14px] mb-1 uppercase tracking-wider">Deployment Complete</p>
          ...
          <div className="bg-white border border-black/[0.08] rounded-3xl p-8 shadow-sm">
            <h3 className="text-slate-900 font-bold text-[13px] uppercase tracking-widest mb-10">Installation Pipeline</h3>
                  ...
                  <div key={step.key} className="flex gap-6">
                    <div className="flex flex-col items-center">
                      ...
                      {index < STEPS.length - 1 && (
                        <div className={`w-[2px] h-12 my-1 ${isCompleted ? 'bg-emerald-500/30' : 'bg-slate-100'}`} />
                      )}
                    </div>

                    <div className={`pb-10 flex-1 ${index === STEPS.length - 1 ? 'pb-0' : ''}`}>
                      ...
                      <p className={`text-[13px] leading-relaxed ${isCurrent ? 'text-slate-500 font-medium' : 'text-slate-400'}`}>
                        {step.desc}
                      </p>
                    </div>
                  </div>
          ...
          {installation.technician && (
            <div className="bg-white border border-black/[0.08] rounded-3xl p-8 shadow-sm">
              <h3 className="text-slate-900 font-bold text-[13px] uppercase tracking-widest mb-6">Field Engineer</h3>
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 rounded-2xl bg-slate-100 border border-black/[0.03] text-slate-900 flex items-center justify-center text-[15px] font-bold flex-shrink-0">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-[13px] font-mono lowercase tracking-tight">{installation.technician.email}</p>
                </div>
              </div>
              {installation.notes && (
